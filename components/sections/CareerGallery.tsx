        ...
        <div className="mt-14 grid grid-cols-1 gap-4 lg:grid-cols-3 lg:grid-rows-2">
          {PHOTOS.map((photo, i) => (
            <motion.div
              ...
              className={`group relative min-h-[220px] overflow-hidden rounded-3xl border border-white/[0.08] transition-colors duration-500 hover:border-[#00D4A5]/30 ${photo.span}`}
            >
              <Image
                src={photo.src}
                alt={photo.alt}
              ...
              />
              <div className="absolute inset-0 bg-gradient-to-t from-[#0A0A0A] via-[#0A0A0A]/15 to-transparent transition-opacity duration-500 group-hover:from-[#0A0A0A]/90" />
              <div className="absolute bottom-0 left-0 right-0 translate-y-1 p-6 transition-transform duration-500 group-hover:translate-y-0">
